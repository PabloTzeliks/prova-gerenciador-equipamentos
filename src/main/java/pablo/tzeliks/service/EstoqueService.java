...
public class EstoqueService implements CrudEstoqueInterface<EquipamentoDTO>, MovimentacaoEstoqueInterface, RelatorioEstoqueInterface<EquipamentoDTO>, BuscaEstoqueInterface<EquipamentoDTO> {
    ...
    @Override
    public synchronized void adicionarUnidadesEstoque(Codigo codigo, int quantidade) throws MovimentacaoEstoqueException {
        if (quantidade <= 0) throw new MovimentacaoEstoqueException("Quantidade inválida.");
        Equipamento e = acharPorCodigoEntidade(codigo);
        if (e == null) throw new MovimentacaoEstoqueException("Equipamento não encontrado.");
        e.setQuantidade(e.getQuantidade() + quantidade);
    }

    @Override
    public synchronized void retirarUnidadesEstoque(Codigo codigo, int quantidade) throws MovimentacaoEstoqueException {
        if (quantidade <= 0) throw new MovimentacaoEstoqueException("Quantidade inválida.");
        Equipamento e = acharPorCodigoEntidade(codigo);
        if (e == null) throw new MovimentacaoEstoqueException("Equipamento não encontrado.");
        int novo = e.getQuantidade() - quantidade;
        if (novo < 0) throw new MovimentacaoEstoqueException("Quantidade insuficiente.");
        e.setQuantidade(novo);
    }
    ...
    private EquipamentoDTO toDto(Equipamento equipamento) {
        ...
        if (equipamento instanceof MotorEletrico) {
            ...
            return mapper.toDTO((PainelControle) equipamento);
        } else {
